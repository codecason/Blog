...
| VAE组件      | 数学对应               | 具体形式/含义                                                |
| ------------ | ---------------------- | ------------------------------------------------------------ |
| **真实后验** | 变分法中的真实极值函数 | $$p(z\parallel x)$$- 我们想得到但很难计算的                  |
| **变分分布** | 变分法中的试探函数     | $$q_\phi(z\parallel x) = \mathcal{N}(z; \mu_\phi(x), \sigma_\phi(x))$$ |
| **ELBO目标** | 变分法中的泛函         | $$\mathcal{L}(\phi, \theta) = \mathbb{E}_{q_\phi}[\log p_\theta(x\parallel z)] - D_{KL}(q_\phi(z\parallel x) \parallel p(z))$$ |
